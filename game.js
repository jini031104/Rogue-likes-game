...
class Player {
  constructor(isRun, maxHP) {
    ...
    this.maxAttack = 10;
    this.attack = 0;
    this.bubbleAttackSuccess = 25;
  }

  Attack(logs) {
    // 플레이어의 공격
    // 최소 ~ 최대 사이의 공격을 가한다.
    this.attack = Math.floor(Math.random() * (this.maxAttack - this.minAttack + 1)) + this.minAttack;
    logs.push(chalk.green(`플레이어의 공격! ${this.attack}의 피해를 입혔다!`));
  }

  DublleAttack(logs) {
    let success = Math.floor(Math.random() * 100); // 0 ~ 99
    if(success <= this.bubbleAttackSuccess) {
      logs.push(chalk.green(`플레이어의 연속 공격!!!!`));
    } else {
...
function displayStatus(stage, player, monster) {
  ...
  console.log(
    ...
    chalk.blueBright(`| 플레이어 정보 - HP: ${player.hp} ATTACK: ${player.minAttack}~${player.maxAttack} `) +
...
const battle = async (stage, player, monster) => {
  ...
  while (player.hp > 0) {
    ...
    console.log(chalk.green(`\n1. 공격  2. 연속 공격(${player.bubbleAttackSuccess}%)  3. 방어  4. 도망`));
